            ...
            try (DirectoryStream<Path> ds = Files.newDirectoryStream(packsDir)) {
                for (Path p : ds) {
                    try {
                        ...
                        boolean accepted =
                                Files.isDirectory(p) || visible.toLowerCase().endsWith(".zip");
                        ...
                        LogManager.getLogger()
                                .warning("resourcepacks: read meta failed for " + p + ": " + exInner.getMessage());
                    }
                }
            } catch (Exception ex) {
                Platform.runLater(() -> ErrorPanel.showError("Ошибка загрузки списка ресурс-паков", ex.getMessage()));
                return;
            }
                ...
                for (PackMeta m : metas) {
                    ...
                    toggle.selectedProperty().addListener((obs, oldVal, val) -> {
                        try {
                            ...
                            ErrorPanel.showError("Не удалось переключить ресурс-пак", ex.getMessage());
