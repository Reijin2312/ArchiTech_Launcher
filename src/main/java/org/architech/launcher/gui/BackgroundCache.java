...
public final class BackgroundCache {
                    ...
                    () -> {
                        try {
                            ...
                            LogManager.getLogger()
                                    .warning("BackgroundCache: read failed for " + p + " : " + e.getMessage());
                            return null;
                        }
                    },
            ...
            return readBytes.thenCompose(bytes -> {
                ...
                Platform.runLater(() -> {
                    try {
                        ...
                        LogManager.getLogger()
                                .warning("BackgroundCache: decode failed for " + p + " : " + e.getMessage());
    ...
    public static void apply(Path p, Region region) {
        ...
        if (p == null || !Files.exists(p)) {
            Platform.runLater(
                    () -> region.setStyle("-fx-background-color: linear-gradient(to bottom, #1e1e1e, #2a2a2a);"));
            return;
        }
        String k = p.toAbsolutePath().toString();
        ...
        loadImageFuture(p)
                ...
                .exceptionally(ex -> {
                    LogManager.getLogger()
                            .warning("BackgroundCache.apply failed for " + p + " : "
    ...
    private static void applyImage(Region region, Image img) {
        ...
        Platform.runLater(() -> {
            ...
            BackgroundImage bi = new BackgroundImage(
                    img, BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT, BackgroundPosition.CENTER, bs);
